class createTable:
    ...
    def build_table(self):
        ...
        self.isPk = input(
            "Esta tabla tiene llave primaria responda si o no").lower()
        if (self.isPk == "si"):
            print(
                "por defecto este programa convierte el primer campo que ingrese como llave primaria")
            self.cantidad_campos = int(
                input("Digite la cantidad de campos de la bd"))
            self.i = 0
    ...
    def send_table(self):
        ...
        self.con.commit()
        self.con.close()"""
        self.field = str(self.keys_values).replace(':', ' ').replace(
            "'", "").replace('}', ')').replace('{', '(')
        self.my_query = "CREATE TABLE " + \
            self.nameTable + " {}".format(self.field)
        try:
